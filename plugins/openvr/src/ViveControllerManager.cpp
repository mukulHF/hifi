...
void ViveControllerManager::InputDevice::handleTrackedObject(uint32_t deviceIndex, const controller::InputCalibrationData& inputCalibrationData) {
    ...
    if (_system->IsTrackedDeviceConnected(deviceIndex) &&
        ...
        if (_nextSimPoseData.vrPoses[deviceIndex].eTrackingResult == vr::TrackingResult_Running_OK || _nextSimPoseData.vrPoses[deviceIndex].eTrackingResult == vr::TrackingResult_Calibrating_OutOfRange || _nextSimPoseData.vrPoses[deviceIndex].eTrackingResult == vr::TrackingResult_Calibrating_InProgress) {
